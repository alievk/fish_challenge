...
def batch_iou_circle(many, one):
  eps = 1e-9
  r = np.minimum(many[:,2], one[2])
  R = np.maximum(many[:,2], one[2])
  d_orig = np.sqrt(np.square(many[:,0] - one[0]) + np.square(many[:,1] - one[1]))
  d = np.clip(d_orig, R - r + 1e-3, R + r - 1e-3)
  rr = np.square(r)
  RR = np.square(R)
  dd = np.square(d)
  cos1 = np.clip((dd + rr - RR)/(2.*d*r + eps), -1., 1.)
  cos2 = np.clip((dd + RR - rr)/(2.*d*R + eps), -1., 1.)
  intersection = \
      rr * np.arccos(cos1) + \
      RR * np.arccos(cos2) + \
      -0.5 * np.sqrt(np.maximum((-d + r + R) * (d - r + R) * (d + r - R) * (d + r + R), 0.))
...
def nms_circle(boxes, probs, threshold):
  """Non-Maximum supression.
  Args:
    boxes: array of [cx, cy, w, h] (center format)
    probs: array of probabilities
  ...
  for i in range(len(order)-1):
    ovps = batch_iou_circle(boxes[order[i+1:]], boxes[order[i]])
    for j, ov in enumerate(ovps):
      if ov > threshold:
        keep[order[j+i+1]] = False
  return keep
