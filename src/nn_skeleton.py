...
class ModelSkeleton:
  ...
  def __init__(self, mc):
    ...
    self.ph_box_delta_input = tf.placeholder(
        tf.float32, [mc.BATCH_SIZE, mc.ANCHORS, mc.SHAPE_DIM], name='box_delta_input')
    # Tensor used to represent bounding box coordinates.
    self.ph_box_input = tf.placeholder(
        tf.float32, [mc.BATCH_SIZE, mc.ANCHORS, mc.SHAPE_DIM], name='box_input')
  ...
  def _add_interpretation_graph(self):
    ...
    with tf.variable_scope('interpret_output') as scope:
      ...
      self.pred_box_delta = tf.reshape(
          ...
          [mc.BATCH_SIZE, mc.ANCHORS, mc.SHAPE_DIM],
          name='bbox_delta'
      )
    ...
    with tf.variable_scope('bbox') as scope:
      delta_x, delta_y, delta_r = tf.unstack(
          self.pred_box_delta, axis=2)

      anchor_x = mc.ANCHOR_BOX[:, 0]
      anchor_y = mc.ANCHOR_BOX[:, 1]
      anchor_r = mc.ANCHOR_BOX[:, 2]

      shape_center_x = tf.identity(
          anchor_x + delta_x * anchor_r, name='bbox_cx')
      shape_center_y = tf.identity(
          anchor_y + delta_y * anchor_r, name='bbox_cy')
      shape_r = tf.identity(
          anchor_r * util.safe_exp(delta_r, mc.EXP_THRESH),
          name='bbox_width')

      self.det_boxes = tf.transpose(
          tf.stack([shape_center_x, shape_center_y, shape_r]),
          (1, 2, 0), name='bbox'
      )

      self._activation_summary(delta_x, 'delta_x')
      self._activation_summary(delta_y, 'delta_y')
      self._activation_summary(delta_r, 'delta_r')

      self._activation_summary(shape_center_x, 'shape_cx')
      self._activation_summary(shape_center_y, 'shape_cy')
      self._activation_summary(shape_r, 'shape_r')
    ...
    with tf.variable_scope('IOU'):
      def _tensor_iou(circle1, circle2):
        ...
        with tf.variable_scope('intersection'):
          ...
          intersection = rr * tf.acos(cos1) + \
                         RR * tf.acos(cos2) + \
                         -0.5 * tf.sqrt(tf.maximum((-d + r + R) * (d - r + R) * (d + r - R) * (d + r + R), 0.))

        with tf.variable_scope('union'):
          union = np.pi*(tf.square(r1) + tf.square(r2)) - intersection

        return intersection/(union+eps) * tf.reshape(self.input_mask, [mc.BATCH_SIZE, mc.ANCHORS])
      ...
      self._activation_summary(self.ious, 'ious')
  ...
  def _add_loss_graph(self):
      ...
      self.conf_loss = tf.reduce_mean(
          tf.reduce_sum(
            tf.square(self.ious - self.pred_conf)
              * (input_mask*mc.LOSS_COEF_CONF_POS/(self.num_objects + mc.EPSILON)
    ...
    with tf.variable_scope('bounding_box_regression') as scope:
      ...
      #     name='bbox_loss'
      # )
      x = self.input_mask*(self.pred_box_delta - self.box_delta_input)
      l1 = tf.abs(x)
      mask = tf.to_float(l1 < 1.)
      softL1 = (l1 - 0.5) * (1 - mask) + 0.5 * tf.square(x) * mask
      self.bbox_loss = tf.truediv(
          tf.reduce_sum(
              mc.LOSS_COEF_BBOX * softL1
          ),
          self.num_objects,
          name='bbox_loss'
      )
  ...
  def _add_viz_graph(self):
    ...
    mc = self.mc
    scale = 0.5
    ih, iw = int(mc.IMAGE_HEIGHT * scale), int(mc.IMAGE_WIDTH * scale)
    self.image_to_show = tf.placeholder(
        tf.float32, [None, ih, iw, 3],
        name='image_to_show'
    )
  ...
  def filter_prediction(self, boxes, probs, cls_idx):
    ...
    for c in range(mc.CLASSES):
      ...
      keep = util.nms_circle(boxes[idx_per_class], probs[idx_per_class], mc.NMS_THRESH)
