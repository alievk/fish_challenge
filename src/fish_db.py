...
class fish_db(object):
  ...
  @staticmethod
  def augment(im, cx, cy, r, p_dropout=0.5, p_fliplr=0.5, p_flipud=0.5, p_blur=0.5, p_brightness=0.5, p_contrast=0.5):
    has_dropout = p_dropout > np.random.rand(1)
    ...
    # dropout
    if has_dropout:
      l,r,u,d = max(0,cx-r), min(1279,cx+r), max(0,cy-r), min(719,cy+r)
      l,r,u,d = [int(x) for x in (l,r,u,d)]
      im[u:d,l:r,:] = iaa.CoarseDropout(p=(.1, .2), size_percent=(.01, .05), deterministic=True).augment_image(im[u:d,l:r,:])
    ...
    return im, (has_dropout, has_fliplr, has_flipud, has_blur, has_brightness, has_contrast)
  ...
  def read_batch(self, shuffle=True):
    ...
    for idx in batch_idx:
      ...
      if mc.DATA_AUGMENTATION:
        p_dropout = 0. if label == 'species_none' else .5
        im, aug_flag = self.augment(im, cx, cy, r, p_dropout=p_dropout)
        _, has_fliplr, has_flipud, _, _, _ = aug_flag
      else:
        has_fliplr, has_flipud = False, False
